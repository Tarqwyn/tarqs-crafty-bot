...
import { fetchGuildRoster, fetchCharacterProfessions } from "../services/blizzard-api";
import { cleanCharacterName } from "../services/utils";

const GUILD_COLLECTION = "guild_members";

async function cleanupOldGuildMembers(collection: any, currentMembersSet: Set<string>) {
    console.log("🗑️ Checking for old members to remove...");

    const storedMembers: { character_realm: string }[] = await collection
        .find({}, { projection: { character_realm: 1 } })
        .toArray();
    ...
    console.log(`🗑️ Removing ${membersToDelete.length} old members from database...`);
    await collection.deleteMany({ character_realm: { $in: membersToDelete } });
    console.log("✅ Cleanup complete! Old members removed.");
}


export async function updateGuildMembers(guildName: string, realm: string, accessToken: string) {
    ...
    console.log(`✅ Retrieved ${roster.members.length} guild members. Syncing to DocumentDB...`);
    ...
    for (const member of roster.members) {
        try {
            ...
            const professions = await fetchCharacterProfessions(accessToken, charName, charRealm);
                ...
                updateOne: {
                    ...
                    update: {
                        $set: {
                            ...
                            khaz_algar_professions: professions || [] // Empty array if no professions
                        }
                    },
                    upsert: true
                }
    ...
    if (bulkUpdateOps.length > 0) {
        console.log(`⚡ Performing bulk update for ${bulkUpdateOps.length} members...`);
