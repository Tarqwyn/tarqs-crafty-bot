...
async function fetchWowheadReagents(spellId) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `❌ Failed to fetch Wowhead spell data for spell ${spellId}`,
      );
    ...
    const optionalReagentsTable = $("h2:contains('Optional Reagents')").next(
      "table.icon-list",
    );
    ...
    console.log(
      `✅ Found ${reagents.length} Reagents and ${optionalReagents.length} Optional Reagents for spell ${spellId}.`,
    );
    return { reagents, optionalReagents };
  } catch (error) {
    console.error(
      `❌ Error fetching Wowhead reagents for spell ${spellId}:`,
      error.message,
    );
    return { reagents: [], optionalReagents: [] };
  }
...
async function fetchItemMediaUrl(itemId) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `❌ Failed to fetch media for item ${itemId} (Status: ${response.status})`,
      );
      return undefined;
    }
  ...
  try {
    ...
    if (jsonText) {
      try {
        ...
        console.warn(
          `⚠️ Warning: Invalid JSON for ${recipeName}. Skipping Spell ID.`,
        );
      }
    } else {
...
async function fetchRecipesForTiers(khazAlgarTiers) {
  ...
  const UTILITY_RECIPES = [
    ...
    "Ominous Call",
    "Gleaming Chaos",
    "Volatile Coalescence",
    "Ominous Coalescence",
    "Gleaming Shatter",
    "Mercurial Coalescence",
    "Gleaming Glory",
    "Scour Through Scrap",
    ...
    "Transcribe: Vivacity",
    "Transcribe: Symbiosis",
    "Transcribe: Radiance",
  ];

  for (const { professionName, professionId, tierId } of Object.values(
    khazAlgarTiers,
  )) {
    ...
    console.log(
      `🔹 Fetching recipes for ${professionName} (Profession ID: ${professionId}, Tier ID: ${tierId})`,
    );
    ...
    if (professionRecipes.length > 0) {
      console.log(
        `✅ Found ${professionRecipes.length} recipes for ${professionName}`,
      );
...
async function fetchKhazAlgarTiers() {
  ...
  for (const profession of data.professions) {
    ...
    const tierInfo = await fetchProfessionDetails(
      profession.id,
      profession.key.href,
    );
    if (tierInfo) {
...
async function generateKhazAlgarRecipeJSON() {
  ...
  try {
    ...
    console.log(
      `✅ Saved ${allRecipes.length} Khaz Algar recipes to ${OUTPUT_PATH}`,
    );
  } catch (error) {
